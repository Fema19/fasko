...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.users.update', $user) }}" class="bg-white p-6 rounded shadow">
        ...
        <div class="mb-4">
            ...
            <select name="role" class="w-full border rounded px-3 py-2 @error('role') border-red-500 @enderror" onchange="toggleRoomSelect()" required>
        ...
        <div class="mb-4" id="room-select" style="display: {{ old('role', $user->role) === 'guru' ? 'block' : 'none' }}">
            ...
            <select name="room_id" class="w-full border rounded px-3 py-2 @error('room_id') border-red-500 @enderror">
                ...
                @foreach ($rooms as $room)
                    <option value="{{ $room->id }}" {{ old('room_id', $user->room_id) == $room->id ? 'selected' : '' }}>{{ $room->name }}</option>
                @endforeach
            </select>
            @error('room_id')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
...
<script>
    function toggleRoomSelect() {
        ...
        roomSelect.style.display = roleSelect.value === 'guru' ? 'block' : 'none';
    }
</script>
@endsection
