...
<div class="max-w-3xl">
    <div class="flex items-start justify-between mb-4">
        <div>
            <h1 class="text-xl font-semibold text-slate-900">Edit Kategori</h1>
            <p class="text-xs text-slate-500">Perbarui nama atau tipe kategori.</p>
        </div>
        <a href="{{ route('guru.categories.index') }}" class="text-xs text-slate-600 hover:text-slate-900">Kembali</a>
    </div>

    <div class="bg-white border rounded-lg p-5">
        <form method="POST" action="{{ route('guru.categories.update', $category) }}">
            @csrf
            @method('PUT')

            <div class="mb-4">
                <label class="block text-sm font-semibold text-slate-700 mb-1">Nama Kategori</label>
                <input name="name" value="{{ old('name', $category->name) }}" class="w-full border px-3 py-2 rounded text-sm focus:outline-none focus:ring focus:ring-slate-200">
                @error('name')
