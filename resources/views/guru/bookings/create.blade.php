...
<div class="max-w-5xl">
    <div class="flex items-start justify-between mb-4">
        <div>
            ...
            <p class="text-xs text-slate-500">Telusuri fasilitas, lalu isi jadwal booking.</p>
        </div>
        <a href="{{ route('guru.bookings.index') }}" class="text-xs text-slate-600 hover:text-slate-900">Kembali</a>
    </div>

    <div class="bg-white p-5 rounded-lg border space-y-4">
        @if ($errors->any())
        ...
        <form method="GET" class="grid md:grid-cols-3 gap-3 text-sm">
            <div>
                ...
                <input type="text" name="q" value="{{ $filters['q'] ?? '' }}" class="w-full border rounded px-3 py-2 text-sm focus:outline-none focus:ring focus:ring-slate-200" placeholder="Nama / deskripsi">
            </div>
            <div>
                <label class="block mb-1 text-slate-700">Kategori</label>
                <select name="category_id" class="w-full border rounded px-3 py-2 text-sm focus:outline-none focus:ring focus:ring-slate-200">
            ...
            <div class="flex items-end gap-2">
                <button class="px-4 py-2 bg-slate-900 text-white rounded-md text-sm">Filter</button>
                <a href="{{ route('guru.bookings.create') }}" class="px-3 py-2 text-xs border rounded text-slate-700 hover:bg-slate-50">Reset</a>
            </div>
        </form>

        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-3">
            @forelse($facilities as $f)
                <div class="border rounded-lg overflow-hidden hover:shadow-sm transition" data-facility="{{ $f->id }}">
        ...
        <form action="{{ route('guru.bookings.store') }}" method="POST" class="space-y-4" id="bookingForm">
            ...
            <div class="p-3 border rounded bg-slate-50 text-sm">
            ...
            <div class="grid md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="datetime-local" name="start_time" value="{{ old('start_time') }}" class="w-full border px-3 py-2 rounded text-sm focus:outline-none focus:ring focus:ring-slate-200" required>
                </div>
                <div>
                    <label class="block mb-1 text-sm text-slate-700">Selesai</label>
                    <input type="datetime-local" name="end_time" value="{{ old('end_time') }}" class="w-full border px-3 py-2 rounded text-sm focus:outline-none focus:ring focus:ring-slate-200" required>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="reason" rows="3" class="w-full border px-3 py-2 rounded text-sm focus:outline-none focus:ring focus:ring-slate-200" placeholder="Contoh: rapat, presentasi">{{ old('reason') }}</textarea>
            </div>

            <div class="pt-2 flex gap-2">
                <button class="px-4 py-2 bg-slate-900 text-white rounded-md text-sm hover:bg-slate-800" id="submitBooking">Kirim Booking</button>
                <a href="{{ route('guru.bookings.index') }}" class="px-4 py-2 text-sm border rounded text-slate-700 hover:bg-slate-50">Batal</a>
            </div>
        </form>
    </div>
</div>
